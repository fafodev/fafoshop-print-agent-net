# AGENTS.md — Hướng Dẫn Cho Cursor (`fafoshop-print-agent-net`, local print agent Windows)

> Cursor tự động đọc file này khi làm việc trong folder
> `fafoshop-print-agent-net/`. Đây là **bản tương đương AGENTS.md** của
> `.claude/CLAUDE.md` trong cùng project (nội dung giữ đồng bộ về ý nghĩa —
> `.claude/CLAUDE.md` vẫn còn nguyên, dùng cho Claude Code nếu cần). Xem
> thêm `../AGENTS.md` (gốc workspace) cho ngữ cảnh cross-project.

Local print agent chạy trên máy POS **Windows** (.NET 8, C#,
`System.Drawing.Printing`/GDI+) — nhận lệnh in từ frontend `fafoshop` (cùng
workspace này, `../fafoshop`) qua HTTP nội bộ (`127.0.0.1:9199`), in thẳng ra
máy in, không qua popup trình duyệt, không phụ thuộc phần mềm in ấn thứ 3
(kiểu QZ Tray).
...
- **CÙNG HTTP contract 100% với bản Java (`fafoshop-print-agent`)** —
  ...
  bản nào. **LƯU Ý workspace này KHÔNG có project `fafoshop-print-agent`
  (bản Java/macOS)** — nếu cần đối chiếu/đổi hành vi cho khớp cả 2 bản, bản
  Java nằm ở workspace gốc `D:\00.SOURCE_WEB\fafoshop-workspace\fafoshop-print-agent`,
  ngoài phạm vi workspace này. Đổi hành vi ở đây mà cần đổi cả bản Java
  tương ứng thì phải báo người dùng để cân nhắc sửa ở workspace gốc.
...
- Toàn bộ nội dung mới (comment, XML doc `<summary>`, message lỗi, log
  ...
  method vẫn theo quy ước C# bình thường (tiếng Anh, PascalCase). KHÔNG có
  ngoại lệ tiếng Anh nào, kể cả log (khác `fafoshop-api`, xem
  `../fafoshop-api/AGENTS.md`).
...
`bin/`/`obj/` KHÔNG được copy sang workspace này (artifact build, tái tạo lại
bằng `dotnet build`) — chạy `dotnet build` trước khi `dotnet run --no-build`
lần đầu trong workspace này.
